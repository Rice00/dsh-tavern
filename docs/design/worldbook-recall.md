...
常驻表示每轮启用，不等于固定文本。含 `{{random::…}}`、`{{roll …}}` 或显式 `Math.random` 的条目所在插入位置整体进入本轮 ForegroundFrame；同位置条目的顺序和标签结构保持不变。其余常驻内容沿用固定前缀。随机常驻不进入非常驻预算、筛选与十轮冷却。

正文准备为本轮保存随机种子，每个条目使用独立随机序列。重复投影、同轮重试及后台读取复用种子；下一轮刷新。历史 Frame 保存当轮实际文本，不回写旧轮次。世界书骰子宏支持 NdM 和整数加减修正。

此分类识别上述显式随机语法，不是任意脚本数据流分析；跨条目变量间接传递随机值、日期等其他变化来源尚不在本次识别范围内。同位置固定内容随随机条目一起追加，是保护标签结构的取舍。
