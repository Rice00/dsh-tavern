...
常驻表示每轮启用，不等于固定文本。含 `{{random::…}}`、`{{roll …}}`、显式 `Math.random`，或明确读取会话状态（如 `getMessageVar`、`getvar`、`variables`）的条目进入本轮 ForegroundFrame；同位置的其他固定条目仍留在前缀。跨条目起止标签包裹随机或非常驻条目时，若边界是独立的纯标签条目，则固定正文留在前缀，动态正文随轮追加；两侧各保留完整标签，不重复固定正文。标签与说明混写或由脚本产生时，不强行拆分，仍保留原有区间。其余常驻内容沿用固定前缀。随机常驻不进入非常驻预算、筛选与十轮冷却。
