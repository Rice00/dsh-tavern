...
- **旧版修复：** 新安装器会识别旧便携版的 `%LOCALAPPDATA%\DSH-Tavern-Portable`、历史 `D:\Workspace\.DSH-Tavern` 和更早的 `%LOCALAPPDATA%\DSH-Tavern`。点击「修复并启动」即可补建入口；原安装、人物卡、聊天和设置继续使用，不自动搬动数据。旧版运行环境可能仍位于隐藏目录。
