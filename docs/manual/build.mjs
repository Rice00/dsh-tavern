  ...
  const article = page => {
    ...
    if (page.id === 'a01') body = body.replace(/(<h2[^>]*>界面截图<\/h2>)/, `$1${figures}${sourceNote}`)
      ...
      body = body.replace('<p>截图：打开终端</p>', figureList[0] || '')
        .replace('<p>截图：选择配置</p>', (figureList[1] || '') + sourceNote)
    }
