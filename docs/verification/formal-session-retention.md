...
`useLiveTavernView` 使用按 sessionId 绑定的 `useSyncExternalStore`，防止切换后的第一次提交仍使用前一会话快照，导致旧状态面板在新会话名下创建并缓存。回收 owner 时先解除订阅，再淘汰无人订阅的完整视图快照；其他无人订阅的快照最多缓存 10 分钟。淘汰会中止仍在请求的同步，迟到结果不得复活旧缓存，返回会话会重新加载。只依赖事件更新的视图不再启动每秒空转的 watchdog。

本次相关测试 177/177 通过；浏览器夹具保留验证七项通过，额外使用真实 React 连续切换 20 次，确认每次提交的会话与快照一致。页面按钮可运行两组验证。未复现此前标签页崩溃，因此不能将这些结果视为崩溃根因已排除。未改动卡片脚本定时器、生成/结算任务或每个 iframe 的上下文传输协议。
