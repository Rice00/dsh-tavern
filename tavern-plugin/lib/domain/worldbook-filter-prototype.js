// PROTOTYPE: answer whether a threshold-triggered Agent improves recall relevance.
...
export function createWorldbookFilterPrototype({ runAgent, selection }) {
  return async ({ chat, userText, candidates }) => {
    const estimatedTokens = candidates.reduce((sum, item) => sum + item.tokenCost, 0)
    const metrics = { prototype: true, candidateCount: candidates.length, estimatedTokens, thresholds: { count: 5, estimatedTokens: 2000 } }
    ...
    const run = await runAgent({
      task: 'worldbook-filter', persistent: false, sessionId: chat.sessionId,
...
当前玩家意图优先，最近对话仅帮助理解指代和场景。不要因历史顺带提到某个词就保留。辨认否定、排除话题，以及中文跨词误匹配（如“峨眉和药王谷”不表示“和药”炼药）。
