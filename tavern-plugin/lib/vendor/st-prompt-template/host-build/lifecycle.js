...
export function createTemplateLifecycle() {
  ...
  return async function synchronize(snapshot) {
    ...
    if (!changedDefinition && previous && same(previous, chat) && same(worlds, snapshot.worldbooks) && same(features, snapshot.extension_settings.EjsTemplate)) return { synchronized: false }
    ...
    for (let index = 0; index < chat.length; index++) {
      ...
      if (!sourceChanged && !variablesChanged && !changedWorlds && !changedSettings && !changedDefinition) continue
