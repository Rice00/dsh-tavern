function ScriptNavigation(props) {
    ...
    return h("section", { className: "dsh-tavern-status-section", "aria-label": "剧本块列表" },
        h("div", { className: "dsh-tavern-status-label" }, "剧本块"),
        h("p", null, "浏览不改变进度。空闲时点击“从这里继续”，下一轮生效。"),
        h("form", { onSubmit: jump },
            h("input", { type: "number", min: 1, max: page?.totalChunks || 1, step: 1, "aria-label": "剧本块序号", placeholder: "输入块号", value: position, disabled: saving,
                onChange: event => setPosition(event.target.value) }),
            h("button", { type: "submit", disabled: loading || saving || !page?.totalChunks }, "浏览"),
            h("button", { type: "button", disabled: loading || saving, onClick: () => load() }, "回到当前游标")),
        error ? h("p", { role: "alert" }, error) : null,
        notice ? h("p", { role: "status" }, notice) : null,
        props.busy ? h("p", null, "任务进行中，可浏览，暂不能修改游标。") : null,
        loading ? h("p", { role: "status" }, "读取中…") : null,
        page ? h("div", null,
            h("p", null, page.from + "–" + page.to + " / " + page.totalChunks + " 块" + (page.cursor >= page.totalChunks && page.totalChunks ? " · 剧本已结束" : "")),
            page.chunks.map(chunk => h("div", { key: chunk.number, className: "dsh-tavern-script-chunk", "aria-current": chunk.number === page.cursor + 1 ? "step" : undefined,
                style: chunk.number === page.cursor + 1 ? { border: "2px solid var(--primary, #b87530)", borderRadius: 6, padding: 8 } : undefined },
                ...
                h("button", { type: "button", disabled: props.busy || loading || saving || chunk.number === page.cursor + 1, onClick: () => point(chunk.number), "aria-label": "从第 " + chunk.number + " 块继续" }, "从这里继续"))),
            h("button", { type: "button", disabled: loading || saving || page.from <= 1, onClick: () => load(Math.max(1, page.from - 6)) }, "前 10 块"),
