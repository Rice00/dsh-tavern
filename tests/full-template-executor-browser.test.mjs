...
test('已有旧入口缓存时，正式 iframe 执行器按版本加载新版并领取任务', { skip: !process.env.TAVERN_BROWSER_TESTS }, async () => {
  ...
  const server = createServer((req, res) => {
    ...
    if (req.url.startsWith(entry)) {
      ...
      const current = req.url.includes('?v=current')
      res.end(`export const templateHost={};export const createTemplateServices=()=>({});export const createTemplatePanel=()=>({});export async function connectTemplateSession({rpc}){return {context:{},synchronize:async()=>({}),${current ? "processNext:async()=>{await rpc('claimFullTemplateWork',{ready:true});return false}" : ''}}}`)
    } else if (req.url === '/') {
  ...
  try {
    ...
    await page.evaluate(entry => {
      ...
      window.executor = createFullTemplateExecutor({ window, rpc: async (method, args) => {
        ...
        return method === 'getFullTemplateRuntimeInfo' ? { entryUrl: entry + '?v=current' } : {}
      } })
