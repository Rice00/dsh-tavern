...
test('folded context preserves field, message, block and tool order without mutation', () => {
  ...
  assert.deepEqual(Array.from(rows, r=>r.title.split(' · ')[0]), ['tools','messages[0]','messages[1]','messages[2]','messages[3]','system','model'])
  rows.slice(1,5).forEach((row,i)=>assert.equal(JSON.stringify(row.value),JSON.stringify(request.messages[i])))
  assert.equal(rows[0].text,JSON.stringify(request.tools,null,2))
