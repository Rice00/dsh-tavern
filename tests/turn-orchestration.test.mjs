...
test('前台自由故事和剧本模式稳定暴露历史正文检索工具', async () => {
  ...
  assert.deepEqual(await story.orchestrator.visibleTools('session-1'), ['skill', 'tavern_read_skill_reference', 'tavern_recall_history'])
  assert.deepEqual(await script.orchestrator.visibleTools('session-1'), ['skill', 'tavern_read_skill_reference', 'tavern_read_script', 'tavern_recall_history'])
})

test('游戏前台按快照启用联网搜索，卡片工作台始终启用', async () => {
  assert.deepEqual(await harness('story').orchestrator.visibleTools('session-1'), ['skill', 'tavern_read_skill_reference', 'tavern_recall_history'])
  assert.deepEqual(await harness('story', { webSearchEnabled: true }).orchestrator.visibleTools('session-1'), ['skill', 'tavern_read_skill_reference', 'tavern_recall_history', 'web_search'])
  assert.deepEqual(await harness('script', { webSearchEnabled: true }).orchestrator.visibleTools('session-1'), ['skill', 'tavern_read_skill_reference', 'tavern_read_script', 'tavern_recall_history', 'web_search'])
