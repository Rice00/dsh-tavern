...
test('原生 DSH 筛选工具、结算与重启恢复使用同一后台 Session 和固定背景', { skip: !process.env.DSH_BOOT_MODULE, timeout: 30000 }, async t => {
  ...
  assert.doesNotMatch(JSON.stringify(requests[2].messages), /首次筛选/)
  assert.doesNotMatch(JSON.stringify(requests[2].messages), /结算完成/)
  assert.match(JSON.stringify(requests[2].messages), /恢复后筛选/)
