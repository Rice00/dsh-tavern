...
```sh
node testsets/run.mjs 001-duan-yingying-continue --check  # 只检查
node testsets/run.mjs 001-duan-yingying-continue          # 真实模型请求
...
**当前边界：**依赖 MVU 或人物卡浏览器脚本的游戏仍需要正式浏览器脚本运行时。纯 API 尚未托管它，会在发送前明确失败，不跳过脚本冒充通过。普通卡的正文、后台、候选续写已经支持。卡片 Agent 和文生图调用正式能力；卡片 Agent 的资源编辑会作用于正式卡库，需用专门测试卡。
