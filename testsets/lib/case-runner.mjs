...
export function createCaseRunner({ casesRoot, resultsRoot, runtimeHome, launch = spawn }) {
  ...
  async function start(id) {
    if (active?.status === 'running') throw Object.assign(new Error('已有测试正在运行，请等待完成'), { status: 409 })
    ...
    try {
      ...
      child.once('exit', (code, signal) => {
        ...
        if (code !== 0) job.error ||= signal ? '测试进程已中断，请查看报告' : '测试未通过，请查看报告；若无报告，请检查运行环境'
        job.finishedAt = new Date().toISOString()
      })
      return { ...job }
    } catch (error) { if (active === job) active = null; throw error }
  }
  return { cases, start, status: () => active && { ...active } }
}
